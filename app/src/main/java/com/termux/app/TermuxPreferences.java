...
final class TermuxPreferences {
    ...
    static TerminalSession getCurrentSession(TermuxActivity context) {
        String sessionHandle = PreferenceManager.getDefaultSharedPreferences(context.activity).getString(TermuxPreferences.CURRENT_SESSION_KEY, "");
    ...
    void reloadFromProperties(Context context) {
        File propsFile = new File(TermuxService.FILES_PATH + "/.termux/termux.properties");
        if (!propsFile.exists())
            propsFile = new File(TermuxService.FILES_PATH + "/.config/termux/termux.properties");
